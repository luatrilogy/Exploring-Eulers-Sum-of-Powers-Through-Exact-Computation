...
def mitm_solutions(
    ...
    *,
    ...
    # Build r-sum table once (Upgrade B/C + packed)
    ...
    for idx, b in enumerate(b_candidates, 1):
        ...
        for rhs_tup, rhs_sum in enumerate_sums(pows, m - r, b, target=target):
            ...
            def add_solution(lhs_tuple: Tuple[int, ...]) -> None:
...
def run_research_mode() -> None:
    print("\n=== RESEARCH MODE ===")
    print("Goal: MITM search with optional b-sieving, bucketing, packed tables, and primitive grouping.\n")
    ...
    # Auto defaults (Upgrade A/B):
    if split_r is None:
        split_r = 2 if m == 4 else 4 if m == 8 else (m // 2)

    if bucket_mod is None and (k >= 7 or split_r >= 3 or N >= 200):
        bucket_mod = 4096
    ...
    print(f"[info] estimated r-sum tuples ≈ {estimate_nondec_tuple_count(N, r):,}")
    print(f"[info] estimated (m-r)-sum tuples per b up to N ≈ {estimate_nondec_tuple_count(N, m-r):,}")
    print(f"[info] bucket_mod={'none' if bucket_mod is None else bucket_mod}")
    print("[note] Start small for k>=7.\n")
