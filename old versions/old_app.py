# main_updated_rhs_cutoff.py
# Speed-optimized (mathematically legitimate) sum-of-powers search.
#
# Adds a major correctness-preserving optimization:
#   RHS 4-sums are grouped by max_elem and sorted by rhs_sum, allowing an early stop
#   for each b when rhs_sum > b^k (since sums are nonnegative, they can never match).
#
# Still includes:
#   - min_b..max_b range control (and precompute only up to max_b)
#   - optional b-sieve
#   - optional LHS bucketing + packed witnesses
#   - strong progress stats + ETA

import time
import math
from typing import Dict, List, Tuple, Iterable, Optional, Union

RHSEntry = Tuple[int, int, int, Tuple[int, int, int, int]]
RHSGrouped = List[List[RHSEntry]]
...
def precompute_rhs_4sums_grouped(pows: List[int], max_val: int, progress: bool = True) -> RHSGrouped:
    """
    ...
    """
    t0 = time.perf_counter()
    groups: RHSGrouped = [[] for _ in range(max_val + 1)]
    count = 0
...
# ============================================================
# Solver: MITM with grouped RHS cutoff for m=8,r=4
# ============================================================
...
def mitm_solutions(
    ...
    *,
    ...
    if m == 8 and r == 4:
        mod_p = 37     # try 19/37/73
        mod_M = 4096   # try 2048/4096/8192 (power of two)

        lhs_res_p_mask = mfold_sumset_bitmask(residues_of_kth_powers(k, mod_p), r, mod_p)
        lhs_res_M_mask = mfold_sumset_bitmask(residues_of_kth_powers(k, mod_M), r, mod_M)
    ...
    rhs_groups: Optional[RHSGrouped] = None
    if m == 8 and r == 4:
