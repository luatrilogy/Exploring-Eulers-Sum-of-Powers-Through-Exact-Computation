...
//
// Equation: a1^k + ... + a8^k = b^k
// We use MITM split 4+4, but store ONLY fingerprints for LHS 4-sums to cut RAM.
// Exact equality is verified with cpp_int only for candidate fingerprint matches.
//
// This does NOT make N=100k feasible if max_val is also huge (the combinatorics are still N^4).
// It DOES massively reduce memory and prevents bad_alloc; it is the correct engineering step
// before adding heavier math sieves.
//
// Parameters:
//  - k: exponent (works for k=8,9,...)
//  - min_b,max_b: scan range for b
//  - max_val: maximum value allowed for ai terms (IMPORTANT knob; keep modest)
//  - mod_M: must be 2^16 (=65536) currently; used for bucket index
//  - p1,p2: 16-bit primes for extra filtering (defaults chosen)
//  - block_b: process b in blocks to keep working-set stable
//  - max_entries_cap: safety cap on LHS entries to avoid OOM
//
