...
def cpp_m8_solutions(
    ...
    *,
    ...
    """C++ backend for m=8 split 4+4 (fast + exact)."""
    if not HAS_CPP:
    ...
    try:
        raw = euler_ext.search_m8_blocked_sieved(
            k=k,
            min_b=min_b,
            max_b=max_b,
            max_val=max_val,
            ...
            max_entries_cap=0,
            primitive_grouping=primitive_grouping,
    ...
    seen = set()
    for lhs8, b in raw:
        lhs8 = tuple(int(x) for x in lhs8)
        b = int(b)
        key = normalize_solution(lhs8, b) if primitive_grouping else (lhs8, b)
        if key in seen:
            continue
        seen.add(key)
        sols.append((lhs8, b))
...
def cpp_m6_solutions(
    ...
    *,
    ...
    seen = set()
    for lhs6, b in raw:
        lhs6 = tuple(int(x) for x in lhs6)
        b = int(b)
        key = normalize_solution(lhs6, b) if primitive_grouping else (lhs6, b)
        if key in seen:
            continue
        seen.add(key)
        sols.append((lhs6, b))
...
def cpp_dfs2_solutions(
    ...
    *,
    ...
    """
    C++ backend: DFS + residue-sieved 2-sum completion (Redesign B).

    This avoids the O(max_val^4) RAM of the 4+4 MITM table. Memory is ~O(max_val^2).
    It is exact (no false positives) and supports arbitrary m>=4 (including m=8 and m=9).
    """
    if not HAS_CPP:
        raise RuntimeError(f"C++ backend not available: {CPP_IMPORT_ERROR}")
...
def run_research_mode() -> None:
    ...
    have_dfs2 = use_cpp and hasattr(euler_ext, "search_dfs_2sum")

    # Heuristic routing:
    # - 4+4 is usually fastest for m=8 when max_val is small (table fits in RAM).
    # - DFS+2-sum avoids O(max_val^4) memory, so prefer it when max_val is large or m is not covered by a specialized backend.
    if use_cpp and (m == 8 and split_r == 4) and (not have_dfs2 or max_val <= 350):
        print("[backend] Using C++ MITM (m == 8 split 4+4)")
        ...
        print("[backend] Using C++ MITM (m == 6 split 3+3)")
        ...
        print("[backend] Using C++ DFS + 2-sum (Redesign B)")
        sols = cpp_dfs2_solutions(
            k=k,
            m=m,
            max_b=max_b,
            min_b=min_b,
